...
**Note:** Both `type` and `columnType` in referenceAttributes use PascalCase. The Connect API canonical wire value for AI columns is `"Ai"`, but the server is case-insensitive -- `"AI"`, `"Ai"`, `"ai"` all work.
